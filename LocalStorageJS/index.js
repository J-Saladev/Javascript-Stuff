...
function submitheInfo() {
    ...
    if (firstname && lastname && type !== "Choose..." && pickup && dropoff && pickup < dropoff) {
        let user = {
            firstname,
            lastname,
            type,
...
function finalPrice() {
 ...
 function pageLoad (){
    if (window.location.pathname == "/LocalStorageJS/results.html") {
        processResult();
        console.log("Page Load Working");
    } else {
        localStorage.clear();
        console.log ("This is not results... Clearing storage");
    }
 }
